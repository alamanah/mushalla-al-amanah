...
interface Props {
  /** Link yang dibuka tombol "Mulai Live" -- default (undefined) buka
   * YouTube Studio (STUDIO_CREATE_LIVE_URL di useKajianLive.ts), dipakai di
   * Dashboard/desktop. Halaman App/PWA di HP mengisi ini dengan link
   * aplikasi YouTube biasa (lihat AppLive.tsx). */
  liveUrl?: string;
}

/**
 * Daftar kajian & khutbah Jumat khusus HARI INI, dipakai bersama oleh
 * Dashboard > Live (desktop, lewat Live.tsx) dan halaman App/PWA > Live
 * (HP, lewat AppLive.tsx) -- isi & filtering-nya sama persis, cuma beda di
 * mana tombol "Mulai Live" mengarah (lihat prop `liveUrl`). Cuma
 * menampilkan jadwal HARI INI -- kajian tanggal spesifik atau rutin
 * mingguan yang jatuh hari ini, dan khutbah Jumat kalau hari ini kebetulan
 * Jumat -- karena dipakai persis sebelum/saat kegiatannya berlangsung,
 * bukan untuk melihat jadwal ke depan (itu tetap lewat Dashboard >
 * Pengaturan Konten > Kelola Jadwal).
 */
export default function LiveScheduleList({ liveUrl }: Props) {
  ...
  } = useKajianLive("kajian_schedule", kajianItems, loadKajian, (k) => k.specific_date, liveUrl);
  ...
  } = useKajianLive("khatib_jumat_schedule", khatibItems, loadKhatib, (k) => k.tanggal, liveUrl);

  return (
    <div>
      {!isYoutubeLiveConfigured() && (
        <p className="text-xs text-yellow-600 mb-4">
          Deteksi otomatis Live YouTube belum aktif. Tombol "Mulai Live" tetap bisa dipakai.
        </p>
      )}
